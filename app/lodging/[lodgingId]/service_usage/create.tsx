  ...
  const handleCloseService = useCallback(
    async (id: string, currentValue: number) => {
      ...
      try {
        ...
        const result = await roomUsageService.finalizedRoomService({
        ...
        setValue("");
        setActive(null);
        removeRoomUsage((result as IRoomServiceInvoice).id);
      } catch (err) {
      } finally {
    ...
    <View className="flex-1 bg-gray-100">
      ...
      {!loading && roomUsages.length <= 0 ? (
        <EmptyScreen
          ...
          title="Chưa đến kỳ chốt dịch vụ"
          description="Vui lòng quay lại sau khi đến thời điểm chốt dịch vụ."
        />
      ) : (
