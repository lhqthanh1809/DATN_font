...
function Detail() {
  ...
  const service = useMemo(() => {
    if (!invoice) return reference.other;

    return (invoice as IRoomServiceInvoice).service
      ? serviceManagerService.getReferenceService(
          (invoice as IRoomServiceInvoice).service
        )
      : reference.other;
  }, [invoice]);
  ...
  const fetchInvoice = useCallback(async () => {
    ...
    const result = await detail(lodgingId as string, "service", id as string);
  ...
  return (
    <View className="flex-1 bg-white-50">
      <HeaderBack title="Chi tiết hoá đơn dịch vụ" />
        ...
        <Button className="flex-1 flex-col items-stretch py-3 gap-3">
            ...
            <View className="gap-2 flex-1 flex-row">
              ...
              <View className="gap-1">
                <Text className="font-BeVietnamSemiBold text-16">
                  {(invoice as IRoomServiceInvoice)?.service
                    ? service.name
                    : (invoice as IRoomServiceInvoice)?.service_name}
                </Text>
          ...
          <View className="bg-white-50 shadow-soft-xs p-5 rounded-2xl border-1 border-white-100 gap-3">
            ...
            <View className="gap-2">
              {(invoice as IRoomServiceInvoice)?.service_payments &&
                (invoice as IRoomServiceInvoice).service_payments?.map((item) => (
                  <ServicePaymentItem item={item} key={item.id} />
                ))}
            </View>
          </View>
        </Button>
      ...
      {invoice && invoice.amount_paid >= invoice.total_price && (
        <View className="p-3">
...
const ServicePaymentItem: React.FC<{
  item: IServicePayment;
}> = ({ item }) => {
  ...
  return (
    <Button
      ...
      onPress={() =>
        router.push(
          `/payment_history/service/${itemLocal.id}?redirect_from=lodging`
        )
      }
    >
