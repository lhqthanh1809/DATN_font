...
const ImagePicker: React.FC<{
  label?: string;
  required?: boolean;
  onChange: (values: MediaLibrary.AssetInfo[]) => void;
  value: MediaLibrary.AssetInfo[] | string[];
  single?: boolean;
}> = React.memo(({ label, required, onChange, single }) => {
  const { showModal, hideModal } = useUI();
  const [width, setWidth] = useState(0);
  const [selectPhotos, setSelectPhotos] = useState<MediaLibrary.AssetInfo[]>(
    []
  );
  const _COLUMN = 4;
  const _GAP = 8;
  ...
  const openImagePicker = useCallback(async () => {
    ...
    showModal(
      <ViewModalImagePicker
        ...
        selects={selectPhotos}
        setSelects={setSelectPhotos}
        hideModal={hideModal}
      />
    );
  }, [single, selectPhotos, showModal, hideModal]);

  const removeImage = useCallback((data: MediaLibrary.AssetInfo) => {
...
const ViewModalImagePicker: React.FC<{
  ...
  setSelects: React.Dispatch<React.SetStateAction<MediaLibrary.AssetInfo[]>>;
  hideModal: () => void;
  single?: boolean;
}> = ({
  numberColumn = 4,
